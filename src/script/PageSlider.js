...
;(function($, window, document, undefined) {
	...
	PageSlider.prototype.showSec = function(index, direction) {
		if ($('.current').length){ $('.current,.next,.prev').css({'-webkit-transition': null,'-webkit-transform': null}).removeClass('current prev next');}
		var cur, next, prev;
	...
	PageSlider.prototype.init = function(optOrIndex){
			...
			if (!that.inited) {
				$('.section').off('touchstart').on('touchstart', {'that': that}, that.touchStartHandler);
