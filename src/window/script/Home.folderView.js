		...
		init : function(obj){
			...
			$.getJSON(ajaxUrl + '?ac=getMyFolderApp&folderid=' + obj.attr('realid'), function(sc){
				...
				$('.quick_view_container_open').on('click',function(){
					Home.window.create($(this).parents('.quick_view_container').attr('realid'), 'folder');
					$('#quick_view_container_' + $(this).parents('.quick_view_container').attr('realid')).remove();
				});
				Home.folderView.getScrollbar(obj.attr('realid'),isScrollbar);
				Home.folderView.moveScrollbar(obj.attr('realid'));
				Home.app.move();
			});
		},
