...
function execLog(){
	$.post('/files?action=GetExecLog',{},function(logs){
		var lbody = '<textarea readonly="" style="margin: 0;width: 500px;height: 520px;background-color: #333;color:#fff; padding:0 5px" id="exec_log">'+logs+'</textarea>';
		$(".taskcon").html(lbody);
...
function shell_translate_text() {
    ...
    $.get('http://www.bt.cn/api/index/fanyi', { query: selectText }, function (rdata) {
        layer.close(loadT);
