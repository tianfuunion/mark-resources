...
bt.files = {
    ...
    zip: function (dirName, submits, callback) {
        ...
        bt.open({
            ...
            content: '<div class="bt-form pd20 pb70">'
                ...
                + '<span>' + lan.files.zip_to + '</span><input type="text" class="bt-input-text" id="dfile" value="' + dirName + ext + '" placeholder="' + lan.files.zip_to + '" style="width: 75%; display: inline-block; margin: 0 10px 0 20px;" /><span class="glyphicon glyphicon-folder-open cursor" onclick="ChangePath(\'dfile\')"></span>'
                + '</div>'
                + '<div class="bt-form-submit-btn">'
                + '<button type="button" class="btn btn-danger btn-sm btn-title" onclick="layer.closeAll()">' + lan.public.close + '</button>'
    ...
    get_right_click: function (type, path, name) {
        ...
        var options = {
            items: [
                {
                    text: lan.files.file_menu_copy, onclick: function () {
                        _this.copy_file(path)
                    }
                },
                {
                    text: lan.files.file_menu_mv, onclick: function () {
                        _this.cut_file(path)
                    }
                },
                {
                    text: lan.files.file_menu_rename, onclick: function () {
                        _this.rename(path, name)
                    }
                },
                {
                    text: lan.files.file_menu_auth, onclick: function () {
                        _this.set_chmod(0, path)
                    }
                },
                {
                    text: lan.files.file_menu_zip, onclick: function () {
                        _this.zip(path)
                    }
                }
        ...
        if (type == "dir") {
            ...
            options.items.push({
                text: lan.files.file_menu_edit, onclick: function () {
            ...
            options.items.push({
                text: lan.files.file_menu_unzip, onclick: function () {
            ...
            options.items.push({
                text: lan.files.file_menu_img, onclick: function () {
                    _this.show_img(path)
                }
            }, {
                text: lan.files.file_menu_down, onclick: function () {
            ...
            if (!rdata.status) {
                ...
                bt.open({
                    ...
                    content: '<div class="setchmod bt-form pd20 pb70">'
                        + '<pre class="crontab-log" style="overflow: auto; border: 0 none; line-height:28px;padding: 15px; margin: 0; height: 405px; background-color: rgb(51,51,51);color:#f1f1f1">' + (rdata.msg == '' ? '当前日志为空' : rdata.msg) + '</pre>'
...
bt.config =
    {
        close_panel: function (callback) {
            layer.confirm(lan.config.close_panel_msg, {
                title: lan.config.close_panel_title, closeBtn: 2, icon: 13, cancel: function () {
...
bt.firewall = {
    ...
    ping: function (status, callback) {
        ...
        layer.confirm(msg, {
            closeBtn: 2, title: lan.firewall.ping_title, cancel: function () {
    ...
    set_mstsc_status: function (status, callback) {
        ...
        layer.confirm(msg, {
            closeBtn: 2, title: lan.public.warning, cancel: function () {
                if (callback) callback(-1); //取消
            }
        }, function () {
            loading = bt.load(lan.public.the);
...
bt.soft = {
    ...
    install_other: function (data) {
        ...
        var loadT = layer.open({
            ...
            shadeClose: false,
            content: '<style>\
                        .install_three_plugin{padding: 25px 25px 70px;}\
    ...
    un_install: function (name) {
        ...
        _this.get_soft_find(name, function (item) {
            ...
            bt.confirm({
                msg: lan.soft.uninstall_confirm.replace('{1}', title).replace('{2}', version),
