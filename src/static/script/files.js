...
//	$.post('/files?action=Get_Recycle_bin','',function(rdata){
	$.post('/console.php/coss/filemanager?status=4','',function(rdata){
		var body = ''
		switch(type){
			case 1:
...
//			getSpeed('.myspeed');
		...
		var formData = 'action=recycle_clear'+
		'&appid=' + $("#fileInputAppid").val() +
		'&appsecret=' + $("#fileInputAppsecret").val();

		$.post('/api.php/bucket/recycle_clear', formData, function(rdata){
			layer.close(loadT);
			console.info("<==================== Console Json Start ====================> Type:" + gettype(rdata));
			var resultJson = formatJson(rdata);
...
//    Path = path_check(Path);
	...
	$.post('/files?action=GetDir&tojs=GetFiles&p=' + p + '&showRow=' + showRow + search, data, function(rdata) {
		...
		if (rdata.PATH != '/') {
			BarTools += ' <button onclick="BackDir();" class="btn btn-default btn-sm glyphicon glyphicon-arrow-left" title="'+lan.files.return+'"></button>';
		}
		setCookie('Path',rdata.PATH);
		BarTools += ' <button onclick="GetFiles(\'' + rdata.PATH + '\');" class="btn btn-default btn-sm glyphicon glyphicon-refresh" title="'+lan.public.fresh+'"></button> <button onclick="web_shell()" title="'+lan.files.shell+'" type="button" class="btn btn-default btn-sm"><em class="ico-cmd"></em></button>';
		...
		if (isPaste != 'null' && isPaste != undefined) {
			BarTools += ' <button onclick="PasteFile(\'' + (GetFileName(isPaste)) + '\');" class="btn btn-Warning btn-sm">'+lan.files.paste+'</button>';
		}
		...
		if (isBatch == 1 || isBatch == '1'){
			BatchTools += ' <button onclick="BatchPaste();" class="btn btn-default btn-sm">'+lan.files.paste_all+'</button>';
		}
...
//	var oldTable = $(window).height() - $('#tipTools')[0].getBoundingClientRect().height - $('#filePage')[0].getBoundingClientRect().height - $('.footer')[0].getBoundingClientRect().height - 111;
	var oldTable = $(window).height() - $('#tipTools')[0].getBoundingClientRect().height - $('#filePage')[0].getBoundingClientRect().height - 111;
...
function showSeclect(){
	...
	var BatchTools = '';
	if(count > 1){
		BatchTools = '<button onclick="Batch(1);" class="btn btn-default btn-sm">'+lan.files.file_menu_copy+'</button>\
						  <button onclick="Batch(2);" class="btn btn-default btn-sm">'+lan.files.file_menu_mv+'</button>\
						  <button onclick="Batch(3);" class="btn btn-default btn-sm">'+lan.files.file_menu_auth+'</button>\
						  <button onclick="Batch(5);" class="btn btn-default btn-sm">'+lan.files.file_menu_zip+'</button>\
						  <button onclick="Batch(4);" class="btn btn-default btn-sm">'+lan.files.file_menu_del+'</button>'
		$("#Batch").html(BatchTools);
	}else{
		$("#Batch").html(BatchTools);
	}
...
function Batch(type,access){
	...
	var bucketid = $("#fileInputBucketid").val();

	var appid = $("#fileInputAppid").val();
	var appsecret = $("#fileInputAppsecret").val();
    ...
    data += "&fileid=" + encodeURIComponent(JSON.stringify(datas))
...
function CreateDir(type, path) {
	if (type == 1) {
		var title = $("#newDirName").val();
		layer.msg(lan.public.the, {
			icon: 16,
			time: 10000
		});

		var form = 'action=create&title='+encodeURIComponent(title)+
			'&appid=' + $("#fileInputAppid").val() +
			'&appsecret=' + $("#fileInputAppsecret").val()+
			'&folderid=' + $("#fileInputFolderid").val()+
			'&path=' + encodeURIComponent(path + '/' + title);

//		$.post('/files?action=CreateDir', 'path=' + encodeURIComponent(path + '/' + title), function(rdata) {
		$.post('/api.php/bucket/createdir', form, function(rdata) {
			console.info("<==================== Console Json Start ====================> Type:" + gettype(rdata));
			var resultJson = formatJson(rdata);
			console.info(resultJson);
			console.info("<==================== Console Json End ====================>");
...
//			GetFiles($("#DirPathPlace input").val());
	...
	layer.open({
		...
		content: '<div class="bt-form pd20 pb70">\
					<div class="line"><input type="text" class="bt-input-text" name="Name" id="newDirName" value="" placeholder="'+lan.files.dir_name+'" style="width:100%" /></div>\
					<div class="bt-form-submit-btn">\
...
//		$.post('/console.php/coss/file_action', 'action=delete&fileid=' + fileid, function(rdata) {

	var form = 'action=remove&fileid=' + fileid +
	'&appid=' + $("#fileInputAppid").val() +
	'&appsecret=' + $("#fileInputAppsecret").val();

		$.post('/api.php/bucket/remove', form, function(rdata) {
			layer.closeAll();
			// console.info(rdata);
			console.info("<==================== Console Json Start ====================> Type:" + gettype(rdata));
			var resultJson = formatJson(rdata);
			...
			switch(data["status"]){
				case 200:
					console.log("状态码：" + data["status"] + " Reason：" + data["reason"] );
					$.toast(data["reason"], function() {
						$("tr.folderBoxTr[data-fileid='"+fileid+"']").remove();

						if($("tr.folderBoxTr[data-fileid]").siblings().length <= 10){
							window.location.reload();
						}
					});
				break;
...
function DeleteDir(dirName){
	layer.confirm(lan.get('recycle_bin_confirm_dir',[dirName]),{title:lan.files.del_dir,closeBtn:2,icon:3},function(){
		layer.msg(lan.public.the,{icon:16,time:0,shade: [0.3, '#000']});
		$.post('/files?action=DeleteDir', 'bucketid=' + encodeURIComponent(dirName), function(rdata) {
			layer.closeAll();
			layer.msg(rdata.msg, {
				icon: rdata.status ? 1 : 2
			});
...
//		data += '&action=SetBatchData';
		data += '&action=remove';

//		$.post('/api.php/bucket/remove', 'action=remove&fileid=' + fileid, function(rdata) {
		$.post('/api.php/bucket/remove',data,function(rdata){
			console.info("<==================== Console Json Start ====================> Type:" + gettype(rdata));
			var resultJson = formatJson(rdata);
			console.info(resultJson);
 ...
 *
 */
function ReName(type, fileid, fileName) {
	if (type == 1) {
		var path = $("#DirPathPlace input").val();
...
//		$.post('/console.php/coss/file_action', 'action=rename&fileid=' + fileid +"&newname="+$("#newFileName").val()+ '&rename=true', function(rdata) {
		$.post('/api.php/bucket/rename', 'action=rename&fileid=' + fileid +
			"&newname="+$("#newFileName").val()+

			"&appid="+$("#fileInputAppid").val()+
			"&appsecret="+$("#fileInputAppsecret").val()+
			"&bucketid="+$("#fileInputBucketid").val()+

			'&rename=true', function(rdata) {
				layer.closeAll();
				console.info(rdata);
				console.info("<==================== Console Json Start ====================> Type:" + gettype(rdata));
				var resultJson = formatJson(rdata);
	...
	layer.open({
		...
		content: '<div class="bt-form pd20 pb70">\
						...
						<div class="bt-form-submit-btn">\
							<button type="button" class="btn btn-danger btn-sm btn-title" onclick="layer.closeAll()">'+lan.public.close+'</button>\
							<button type="button" id="ReNameBtn" class="btn btn-success btn-sm btn-title" onclick="ReName(1,' + fileid + ',\'' + fileName.replace(/'/,"\\'") + '\')">'+lan.public.save+'</button>\
						</div>\
					</div>'
	});
	$("#newFileName").focus().keyup(function(e){
...
function PasteFile(fileName) {
	...
	$.post('/files?action=CheckExistsFiles',{dfile:path,filename:filename},function(result){
		if(result.length > 0){
			...
			var mbody = '<div class="divtable"><table class="table table-hover" width="100%" border="0" cellpadding="0" cellspacing="0"><thead><th>文件名</th><th>大小</th><th>最后修改时间</th></thead>\<tbody>'+tbody+'</tbody>\</table></div>';
			SafeMessage('即将覆盖以下文件',mbody,function(){
...
function Zip(dirName,submits) {
	...
	layer.open({
		...
		content: '<div class="bt-form pd20 pb70">'
					...
					+'<span>'+lan.files.zip_to+'</span><input type="text" class="bt-input-text" id="dfile" value="'+dirName + '.tar.gz" placeholder="'+lan.files.zip_to+'" style="width: 75%; display: inline-block; margin: 0 10px 0 20px;" /><span class="glyphicon glyphicon-folder-open cursor" onclick="ChangePath(\'dfile\')"></span>'
                    + '</div>'
					+'<div class="bt-form-submit-btn">'
					+'<button type="button" class="btn btn-danger btn-sm btn-title" onclick="layer.closeAll()">'+lan.public.close+'</button>'
...
//	var imgUrl = '/download?filename='+fileName;
	var imgUrl = '/api.php/bucket/get?object='+fileName+'&x-coss-process=style/primitive_image_format';
...
function GetPlay(fileName) {
    ...
    layer.open({
        ...
        area: '500px',
        shadeClose: false,
        content: '<div class="showpicdiv"><video src="' + imgUrl +'" controls="controls" autoplay="autoplay" width="100%" type="video/mp4">\您的浏览器不支持 video 标签。\</video></div>'
    });
    $(".layui-layer").css("top", "30%");
}
...
function UploadFiles() {
	...
	var folderid = $("#fileInputFolderid").val();
	var bucketid = $("#fileInputBucketid").val();

	var appid = $("#fileInputAppid").val();
	var appsecret = $("#fileInputAppsecret").val();

//    bt_upload_file.open(path,folderid, bucketid, null, null, function (path) {
    bt_upload_file.open(path,folderid, appid, appsecret,null, null, function (path) {
//        GetFiles(path);
    });
